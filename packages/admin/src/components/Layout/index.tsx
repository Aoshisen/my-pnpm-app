...
const Layout: FC<PropsWithChildren<LayoutProps>> = ({ children, header, side }) => {
	return (<div id="layout" className="grid grid-rows-[40px_1fr] grid-cols-[200px_10px_1fr] h-full">
		<header className="col-span-full bg-white overflow-y-auto">{header}</header>
		<div className="bg-fuchsia overflow-y-auto">{side}</div>
		<div className="bg-white"></div>
		<div className=" bg-blueGray overflow-y-auto">{children}</div>
	</div>)
};
